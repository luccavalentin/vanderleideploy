...
import { Pencil, Trash2, Plus, TrendingUp, Target, Lightbulb, ArrowLeft } from "lucide-react";
import { standardizeText } from "@/lib/validations";
...
export default function RevenueOptimization() {
  ...
  const createIdeaMutation = useMutation({
    mutationFn: async (data: any) => {
      const { error } = await supabase.from("revenue_optimization_ideas").insert([data]);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["revenue_optimization_ideas"] });
  ...
  const updateIdeaMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const { error } = await supabase.from("revenue_optimization_ideas").update(data).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["revenue_optimization_ideas"] });
  ...
  const deleteIdeaMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("revenue_optimization_ideas").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["revenue_optimization_ideas"] });
  ...
  const handleSubmitIdea = (e: React.FormEvent) => {
    ...
    const data = {
      ...
      estimated_revenue: ideaFormData.estimated_revenue ? parseFloat(ideaFormData.estimated_revenue) : null,
      ...
      actual_revenue: ideaFormData.actual_revenue ? parseFloat(ideaFormData.actual_revenue) : null,
  ...
  const handleEditIdea = (idea: any) => {
    ...
    setIdeaFormData({
      ...
      estimated_revenue: idea.estimated_revenue?.toString() || "",
      ...
      actual_revenue: idea.actual_revenue?.toString() || "",
      notes: idea.notes || "",
    });
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden">
      <PageHeader
        ...
        description="Cadastre ideias e planejamentos para aumentar receitas do negócio"
        action={{
          label: "Nova Ideia",
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Card className="border-2 border-success/30 rounded-2xl shadow-elegant-lg">
          <CardContent className="p-4 sm:p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Receita Estimada</p>
        ...
        <Card className="border-2 border-primary/30 rounded-2xl shadow-elegant-lg">
          <CardContent className="p-4 sm:p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Receita Realizada</p>
        ...
        <Card className="border-2 border-border/50 rounded-2xl shadow-elegant-lg">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              Ideias de Otimização de Receita
            </CardTitle>
        ...
        <Card className="border-2 border-border/50 rounded-2xl shadow-elegant-lg">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              Planejamentos de Otimização de Receita
            </CardTitle>
      ...
      <Dialog open={isIdeaDialogOpen} onOpenChange={(open) => !open && handleCloseIdeaDialog()}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingIdeaId ? "Editar Ideia" : "Nova Ideia de Otimização de Receita"}</DialogTitle>
            <DialogDescription>
              Cadastre uma ideia para aumentar receitas do negócio
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmitIdea} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Ex: Novos Produtos, Upsell, Parcerias, Marketing..."
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="estimated_revenue">Receita Estimada (R$)</Label>
                <Input
                  id="estimated_revenue"
                  type="number"
                  step="0.01"
                  value={ideaFormData.estimated_revenue}
                  onChange={(e) => setIdeaFormData({ ...ideaFormData, estimated_revenue: e.target.value })}
                />
              </div>
              <div className="space-y-2">
            ...
            {ideaFormData.status === "implementada" && (
              <div className="space-y-2">
                <Label htmlFor="actual_revenue">Receita Realizada (R$)</Label>
                <Input
                  id="actual_revenue"
                  type="number"
                  step="0.01"
                  value={ideaFormData.actual_revenue}
                  onChange={(e) => setIdeaFormData({ ...ideaFormData, actual_revenue: e.target.value })}
                />
              </div>
            )}
      ...
      <Dialog open={isPlanDialogOpen} onOpenChange={(open) => !open && handleClosePlanDialog()}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Crie um planejamento detalhado para implementar a otimização de receitas
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmitPlan} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="target_value">Meta de Receita (R$)</Label>
                <Input
                  id="target_value"
                  type="number"
                  step="0.01"
            ...
            {planFormData.status === "concluido" && (
              <div className="space-y-2">
                <Label htmlFor="plan_actual_value">Receita Realizada (R$)</Label>
                <Input
